                    ...
                    foreach ($gov_data as $mkey => $value) { ?>
                        <div class="month <?= ($current_month==$mkey+1) ? 'active_month': 'disable_month'; ?>">
                            ...
                            <div class="month_before">
                                <?php if($mkey==0) { ?>
                                    <a href="<?= Url::to(['/test/calendar/yr='.($yr-1).'/mn=last'])?>"><i class="fa fa-angle-left"></i>Декабрь <?= $yr-1 ?></a>
                                <? } else { ?>
                                    <span class="month_before_button"><i class="fa fa-angle-left"></i><?= $gov_data[$mkey-1][month] ?></span>
                                <? } ?>
                            </div>
                            <div class="month_after">
                                <?php if($mkey==11) { ?>
                                    <a href="<?= Url::to(['/test/calendar/yr='.($yr+1)])?>">Январь <?= $yr+1 ?><i class="fa fa-angle-right"></i></a>
                                <? } else { ?>
                                    <span class="month_after_button"><?= $gov_data[$mkey+1][month] ?><i class="fa fa-angle-right"></i></span>
                                <? } ?>
                            </div>
                            <div class="week">
                                <div>Понедельник</div>
