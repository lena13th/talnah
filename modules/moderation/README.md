...
To enable moderation for a model, use the `app\modules\moderation\ModerationBehavior` behavior and add the `status` and `moderated_by` columns to your model's table.
...
```php
use app\modules\moderation\ModerationBehavior;
...
By default [[app\modules\moderation\ModerationBehavior]] triggers [[app\modules\moderation\ModerationBehavior::EVENT_BEFORE_MODERATION]]
...
```php
use app\modules\moderation\ModerationQuery;
